...
TEST_NAME="VOTERS-QUERY" && timerStart $TEST_NAME
echoInfo "INFO: $TEST_NAME - Integration Test - START"

PROPOSAL_RESULT=$(setPoorNetworkMessages validator asdf)

INTERX_GATEWAY="127.0.0.1:11000"
PROPOSAL_ID_INTERX=$(curl --fail "$INTERX_GATEWAY/api/kira/gov/proposals?all=true&reverse=true" | jq '.proposals | length' | tr -d '"' || exit 1)
