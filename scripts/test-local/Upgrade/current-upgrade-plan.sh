...
TEST_NAME="PLAN-CURRENT-UPGRADE-QUERY" && timerStart $TEST_NAME
echoInfo "INFO: $TEST_NAME - Integration Test - START"

INTERX_GATEWAY="127.0.0.1:11000"
PLAN_DATA_INTERX=$(curl --fail "$INTERX_GATEWAY/api/kira/upgrade/current_plan" | jq '.plan' || exit 1)
PLAN_NAME_INTERX=$(echo $PLAN_DATA_CLI | jq '. | name' || echo "null")
PLAN_PROPOSAL_INTERX=$(echo $PLAN_DATA_CLI | jq '. | proposalID' || echo "null")

PLAN_DATA_CLI=$(showCurrentPlan | jq '.plan')
